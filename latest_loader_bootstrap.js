(()=>{
 ...
 const files=[
  ...
  'content_adapter_v1.js','radar_106_115_priority_v1.js','question_quality_rules_v1.js','canonical_concepts_v1.js','difficulty_ladder_v1.js','concept_mastery_v1.js','personal_priority.js','daily_mission.js','learning_progress.js','navigation_state.js'
 ];
 const loaded=new Set([...document.scripts].map(s=>(s.getAttribute('src')||'').split('?')[0]));
 function load(src){return new Promise(resolve=>{if(loaded.has(src)){resolve();return}const s=document.createElement('script');s.src=src+'?v=20260818s';s.onload=()=>{loaded.add(src);resolve()};s.onerror=()=>resolve();document.body.appendChild(s)})}

(()=>{
 ...
 function termRows(level=''){
  ...
  return high.map(x=>{
   ...
   let wrongN=0,unknownN=0,attemptN=0,correctN=0,unknownAttempts=0;
   related.forEach(q=>{const a=attempts[q.id];if(a){attemptN+=a.total||0;correctN+=a.correct||0;unknownAttempts+=a.unknown||0}if(wrong.has(q.id))wrongN++;if(unknown.has(q.id))unknownN++});
   ...
   return {...x,related,wrongN,unknownN,unknownAttempts,attemptN,acc,personal,priority,rising:!!risingHit,rare:!!rareHit};
  }).sort((a,b)=>b.priority-a.priority||b.unknownN-a.unknownN||b.wrongN-a.wrongN||b.score-a.score);
 }
 function label(x){const a=[];if(x.unknownN)a.push(`❓ ${x.unknownN} 題目前不知道`);if(x.wrongN)a.push(`❌ ${x.wrongN} 題仍在錯題本`);if(x.acc!==null&&x.acc<70)a.push(`📉 相關題正確率 ${x.acc}%`);if(x.yearN>=3)a.push('🔥 十年高頻');if(x.rising)a.push('🚀 近年升溫');if(x.rare)a.push('🧊 高鑑別');if(!x.attemptN)a.push('🆕 尚未建立熟練度');return a.slice(0,5).join('　')}
 function render(level=''){
  ...
  if(meta)meta.textContent=attemptTotal?`已依 ${attemptTotal} 題次作答紀錄重新排序：❓不知道 ${unknownTotal} 題的權重高於 ❌錯題 ${wrongTotal} 題，再疊加十年高頻、近三年升溫與高鑑別度。`:'目前還沒有足夠個人作答紀錄，先依考古題重要度排序；開始刷題後會逐漸變成你的個人榜。';

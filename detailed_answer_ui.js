...
(()=>{
  function mount(){
    ...
    if(termBtn&&!termBtn.dataset.detailedAnswerReady){
      termBtn.dataset.detailedAnswerReady='1';
      termBtn.textContent='📖 看詳細擬答';
      termBtn.addEventListener('click',()=>{
        const name=byId('termName')?.textContent||'';
        const item=(window.TERM_PROMPTS||[]).find(x=>x.term===name);
        if(!item)return;
        byId('termAnswer').innerHTML=`<div class="feedback-title">📝 名詞解釋完整擬答</div><p>${para(item.model_answer||item.definition||'')}</p><div class="feedback-title">🎯 考場寫法提醒</div><p>建議順序：一句話定義 → 時代／文化脈絡 → 2～3項核心特徵 → 代表人物、作品或例證。若題目配分較低，可優先保留定義與兩個最具鑑別力的關鍵字。</p>`;
        byId('termAnswer').classList.remove('hidden');
      });
    }
  }

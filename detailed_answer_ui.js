(()=>{
  ...
  function loadScript(src,flag){const base=src.split('?')[0];if([...document.scripts].some(s=>(s.getAttribute('src')||'').split('?')[0]===base))return;const s=document.createElement('script');s.src=src;s.dataset[flag.replace(/-([a-z])/g,(_,c)=>c.toUpperCase())]='1';s.defer=true;document.body.appendChild(s)}
  function loadTools(){loadScript('latest_loader_bootstrap.js?v=20260818u','latest-loader');loadScript('term-learning.js?v=20260818d','term-learning');loadScript('bad-question.js?v=20260818a','bad-question')}
  if(document.readyState==='loading')window.addEventListener('DOMContentLoaded',()=>{mount();loadTools()});else{mount();loadTools()}
})();
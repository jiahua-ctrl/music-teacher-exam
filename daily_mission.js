(()=>{
 ...
 function build(){const s=load(STATS,{attempts:{},wrong:[],unknown:[],topics:{}}),r=load(SRS,{}),now=new Date(),all=qs(),used=new Set();
  const take=(arr,n)=>{const out=[];for(const q of arr){if(out.length>=n)break;if(!used.has(q.id)){used.add(q.id);out.push(q)}}return out};
  ...
  const parts=[['🧠 到期複習',take(due,5)],['❓ 尚未建立',take(unknown,4)],['❌ 弱點回收',take([...wrong,...weak],4)],['🔥 高頻弱點',take(weak,4)],['🆕 新題探索',take(fresh,3)]];
  let pool=parts.flatMap(x=>x[1]);if(pool.length<20)pool=pool.concat(take(mixed,20-pool.length));return{parts,pool:uniq(pool).slice(0,20)} }
 function render(){const home=document.getElementById('homeView');if(!home||document.getElementById('dailyMission'))return;const m=build(),sec=document.createElement('section');sec.id='dailyMission';sec.className='card section-card';sec.innerHTML=`<div class="section-title"><div><span class="eyebrow">不用自己決定今天刷什麼</span><h3>📅 今日上岸任務</h3></div><span class="pill">${m.pool.length} 題</span></div><p class="muted">依到期複習、❓不知道、錯題與弱點、新題自動配題；會的題目降低比例，不讓熟題一直佔時間。</p><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:8px;margin:12px 0">${m.parts.map(([n,a])=>`<div style="border:1px solid var(--line);border-radius:12px;padding:10px"><b>${n}</b><div style="font-size:22px;font-weight:850;margin-top:4px">${a.length}<small style="font-size:12px;color:var(--muted)"> 題</small></div></div>`).join('')}</div><button id="startDailyMission" class="primary wide">🚀 開始今天的上岸任務</button>`;
